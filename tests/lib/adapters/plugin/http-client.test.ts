...
describe('fetchJson', () => {
  ...
  it('adds Accept and User-Agent headers when not present', async () => {
    ...
    const { fetchJson } = await import('@/lib/adapters/plugin/http-client');
    await fetchJson('https://example.com/api');
    const [, init] = fetchMock.mock.calls[0];
    const headers = init.headers as Headers;
    expect(headers.get('Accept')).toBe('application/json');
    expect(headers.get('User-Agent')).toBe('MC-Vector/2.0.62');
  });
  ...
  it('aborts a request after the timeout', async () => {
    ...
    const { fetchJson, PLUGIN_API_TIMEOUT_MS } = await import('@/lib/adapters/plugin/http-client');

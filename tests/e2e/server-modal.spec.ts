...
test.describe('Server creation modal', () => {
  ...
  test('create server button is visible in sidebar', async ({ page }) => {
    await expect(page.locator('[data-testid="create-server-button"]')).toBeVisible();
  });
  ...
  test('choice modal has new server and import options', async ({ page }) => {
    ...
    await expect(modal.locator('[data-testid="choice-import-server-button"]')).toBeVisible();
  });
  ...
  test('new server button opens add server form', async ({ page }) => {
    ...
    await expect(page.locator('[data-testid="add-server-modal"]')).toBeVisible({ timeout: 5_000 });
  });

...
| **pnpm**    | v10.26.2           | `npm install -g pnpm@10.26.2`     |
...
pnpm check:fix  # Format and auto-fix lint issues
pnpm rustfmt:check
```

[Lefthook](https://github.com/evilmartians/lefthook) runs Oxfmt and Oxlint with staged JavaScript/TypeScript files and runs a check-only Rustfmt gate for staged Rust changes. Use `pnpm rustfmt` to apply Rust formatting explicitly.
...
This creates an unsigned debug build with the same isolated debug product name and bundle identifier as `pnpm tauri:dev`.
...
| `pnpm tauri:dev`       | Full desktop app in isolated debug mode            |
| `pnpm build`           | Build frontend for production                      |
| `pnpm tauri:build`     | Build production Tauri binary                      |
| `pnpm tauri:build:debug` | Build unsigned isolated debug Tauri binary       |
| `pnpm check`           | Run Oxlint + Oxfmt checks                         |
| `pnpm check:fix`       | Format and auto-fix lint issues                   |
| `pnpm lint`            | Run Oxlint                                         |
| `pnpm lint:fix`        | Run Oxlint with auto-fix                          |
| `pnpm fmt`             | Format code with Oxfmt                            |
| `pnpm fmt:check`       | Check Oxfmt output without changes                |
| `pnpm format`          | Compatibility alias for `pnpm fmt`                |
| `pnpm rustfmt`         | Format Rust code (`cargo fmt`)                   |
| `pnpm rustfmt:check`   | Check Rust formatting without changes             |

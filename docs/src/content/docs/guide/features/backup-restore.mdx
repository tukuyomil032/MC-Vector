...
Online backups are supported while the server is running. Minecraft-generated `session.lock` files are automatically excluded from backups. For a static, consistent snapshot of server data, stopping the server before creating the backup is recommended.

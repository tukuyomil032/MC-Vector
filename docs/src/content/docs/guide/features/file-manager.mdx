...
All server file operations are resolved by the Rust backend below the selected server's managed
root. File actions use a server ID and relative path; absolute paths and traversal outside the
managed root are rejected. Importing an external folder uses the native folder picker and copies
the selected content into a managed server directory before it becomes available to the file
manager.

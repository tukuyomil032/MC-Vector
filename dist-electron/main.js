  ...
  setInterval(async () => {
    ...
    for (const [serverId, process2] of activeServers) {
      if (process2 && process2.pid) {
        try {
          const stats = await pidusage(process2.pid);
  ...
  ipcMain.handle("download-server-jar", async (event, serverId) => {
    ...
    try {
      ...
      if (server.software === "Velocity") {
        ...
        const projectMap = { "Paper": "paper", "Waterfall": "waterfall", "LeafMC": "leaf" };
...
servers:
  # Auto-generated`;
      ...
      const existingProxy = allServers.find((s) => s.name === "Proxy-Server");
      if (!existingProxy) {
        const proxyServer = {
...
`);
  ...
  ipcMain.on("open-proxy-help-window", () => {
    ...
    if (process.env.VITE_DEV_SERVER_URL) {
      ...
      proxyHelpWindow.loadURL(`file://${path.join(__dirname$1, "../dist/index.html")}#proxy-help`);
    }

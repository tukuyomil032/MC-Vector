...
function loadConfig() {
  if (fs.existsSync(CONFIG_PATH)) {
    try { return JSON.parse(fs.readFileSync(CONFIG_PATH, 'utf-8')); } catch (e) { console.error(e); }
  }
  return {};
}
...
function loadServersList() {
  ...
  try { return JSON.parse(fs.readFileSync(jsonPath, 'utf-8')); } catch (e) { return []; }
}
...
function writeServerProperties(filePath: string, properties: Map<string, string>) {
  ...
  properties.forEach((value, key) => { content += `${key}=${value}\n`; });
...
function createWindow() {
  ...
  if (process.env.VITE_DEV_SERVER_URL) {
    mainWindow.loadURL(process.env.VITE_DEV_SERVER_URL); mainWindow.webContents.openDevTools();
  } else {
...
app.whenReady().then(() => {
  ...
  ipcMain.handle('add-server', async (_event, serverData) => {
    try {
      ...
      const newServer = {
        id, name: serverData.name, version: serverData.version, software: serverData.software,
        port: serverData.port, memory: serverData.memory, path: serverDir, status: 'offline',
      ...
      return newServer;
    } catch (e) { console.error('Failed to add server:', e); throw e; }
  });
  ...
  ipcMain.handle('delete-server', async (_event, serverId) => {
    try {
      ...
      if (serverToDelete.path && fs.existsSync(serverToDelete.path)) {
        try { fs.rmSync(serverToDelete.path, { recursive: true, force: true }); } catch (e) { console.error(e); }
      }
  ...
  ipcMain.handle('setup-proxy', async (_event, config) => {
    ...
    } catch (err: any) { return { success: false, message: `エラー: ${err.message}` }; }
  });
  ...
  ipcMain.on('stop-server', (event, serverId) => {
    ...
    if (process) {
      ...
      if (process.stdin) process.stdin.write('stop\n');
    } else {
  ...
  ipcMain.on('send-command', (event, { serverId, command }) => {
    ...
    if (process) {
      ...
      if (process.stdin) process.stdin.write(`${command}\n`);
    } else {
  ...
  ipcMain.on('open-settings-window', (_event, currentSettings) => {
    ...
    settingsWindow = new BrowserWindow({
      width: 900, height: 700, parent: mainWindow || undefined, autoHideMenuBar: true,
      webPreferences: { preload: path.join(__dirname, 'preload.mjs'), nodeIntegration: false, contextIsolation: true }
    });
    if (process.env.VITE_DEV_SERVER_URL) settingsWindow.loadURL(`${process.env.VITE_DEV_SERVER_URL}#settings`);
    else settingsWindow.loadURL(`file://${path.join(__dirname, '../dist/index.html')}#settings`);
    settingsWindow.on('closed', () => { settingsWindow = null; });
  });
  ipcMain.on('settings-window-ready', (event) => { if (tempSettingsData) event.sender.send('init-settings-data', tempSettingsData); });
  ipcMain.on('save-settings-from-window', (_event, newSettings) => { if (mainWindow) mainWindow.webContents.send('settings-updated', newSettings); });
  ...
  ipcMain.handle('read-file', async (_event, filePath) => { try { return await fs.promises.readFile(filePath, 'utf-8'); } catch { return ''; } });
  ipcMain.handle('save-file', async (_event, filePath, content) => { try { await fs.promises.writeFile(filePath, content, 'utf-8'); return true; } catch { return false; } });

  // 新規追加: ディレクトリ作成
  ipcMain.handle('create-directory', async (_event, dirPath) => {
    try { await fs.promises.mkdir(dirPath, { recursive: true }); return true; } catch { return false; }
  });
  // 新規追加: 削除
  ipcMain.handle('delete-path', async (_event, targetPath) => {
    try { await fs.promises.rm(targetPath, { recursive: true, force: true }); return true; } catch { return false; }
  });
  // 新規追加: 移動
  ipcMain.handle('move-path', async (_event, srcPath, destPath) => {
    try {
      await fs.promises.rename(srcPath, destPath); return true;
    } catch (e) {
      try {
  ...
  ipcMain.handle('compress-files', async (_event, filePaths, destPath) => {
    try {
      ...
      zip.writeZip(destPath); return true;
    } catch { return false; }
  });
  // 新規追加: 解凍
  ...
  ipcMain.handle('create-backup', async (_event, _id, serverPath) => {
    try {
      ...
      files.forEach(f => {
        ...
        if (fs.statSync(p).isDirectory()) zip.addLocalFolder(p, f); else zip.addLocalFile(p);
      });
  ...
  ipcMain.handle('delete-backup', async (_event, serverPath, backupName) => {
    try {
      const p = path.join(serverPath, 'backups', backupName);
      if (fs.existsSync(p)) { await fs.promises.unlink(p); return true; }
      return false;
    } catch { return false; }
  });
})

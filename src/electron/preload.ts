...
contextBridge.exposeInMainWorld('electronAPI', {
  ...
  onServerStats: (callback: (event: unknown, data: any) => void) => {
    ...
    return () => {
      ipcRenderer.removeListener('server-stats', subscription);
    };
  },
  onServerStatusUpdate: (callback: (event: unknown, data: any) => void) => {
    ...
    return () => {
      ipcRenderer.removeListener('server-status-update', subscription);
    };
  },
  ...
  compressFiles: (files: string[], dest: string, serverId: string) =>
    ipcRenderer.invoke('compress-files', files, dest, serverId),
  extractArchive: (archive: string, dest: string, serverId: string) =>
  ...
  toggleNgrok: (serverId: string, enabled: boolean, token?: string) =>
    ipcRenderer.invoke('toggle-ngrok', serverId, enabled, token),
  onNgrokInfo: (callback: (event: unknown, data: any) => void) => {
    const subscription = (_event: unknown, data: any) => callback(_event, data);
    ipcRenderer.on('ngrok-info', subscription);
    return () => {
      ipcRenderer.removeListener('ngrok-info', subscription);
    };
  },
